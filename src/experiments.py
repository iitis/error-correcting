...
graph = generate_chimera_from_csv(chimera_path)
graph_solved = generate_solved_chimera_from_csv(chimera_path, solution_dict, 2)

graph_fliped = random_spin_flips(graph_solved, 0.05)
print(compute_energy_nx(graph_fliped))
...
if __name__ == "__main__":
    ...
    for i in range(STEPS):
        val_env = ComputeChimera(graph, include_spin=True)
        # val_env.reset()
    ...
    plt.xlabel("steps")
    plt.ylabel("energy")
    plt.title('instance 2 after {} repetitions'.format(STEPS))
    plt.show()
